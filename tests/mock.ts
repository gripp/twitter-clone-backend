...
export const mockUserFindById = jest.fn(async (id: Types.ObjectId) => {
  if (USER_ID.equals(id)) return { _id: USER_ID } as User;

  if (USER_ID_ON_BEHALF.equals(id)) return { _id: USER_ID_ON_BEHALF } as User;
...
export const mockGetTweetById = jest.fn(async (id: Types.ObjectId): Promise<Tweet | null> => {
  if (id.equals(TWEET_ID)) return tweet_1;
  if (id.equals(user_tweet_1._id)) return user_tweet_1;
  return null;
});

export const mockGetByUserId = jest.fn(async (id: Types.ObjectId): Promise<Tweet[] | null> => {
  if (USER_ID_ON_BEHALF.equals(id)) return [user_tweet_1, user_tweet_2];
  return null;
});
...
export const mockFollowings = jest.fn(async (userId: Types.ObjectId): Promise<Follow[] | null> => {
  if (userId.equals(USER_ID)) return [follower_1, follower_2];
  else return null;
});

export const mockFollowCreate = jest.fn(
  async (followerId: Types.ObjectId, followeeId: Types.ObjectId): Promise<Follow | null> => {
